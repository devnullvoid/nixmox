...
Orchestrate the complete deployment workflow for a NixMox module.

Arguments:
    module          The module to deploy (e.g., authentik, caddy, monitoring)

Options:
    -h, --help          Show this help message
    -d, --dry-run       Show what would be done without executing
    -s, --skip-lxc      Skip LXC creation (assume container exists)
...
Examples:
    $0 authentik                    # Full deployment workflow
    $0 authentik --dry-run         # Show what would be done
    $0 authentik --skip-lxc        # Skip LXC creation
...
# Function to check if LXC container exists
check_lxc_exists() {
    local target_host="$1"
    local container_name="$MODULE"
    
    log "Checking if LXC container '$container_name' exists on Proxmox..."
    
    if [[ "$DRY_RUN" == "true" ]]; then
        dry_run_log "Would check if LXC container '$container_name' exists"
        return 0
    fi
    
    # Try to connect to the container
    if ssh -o ConnectTimeout=5 "$target_host" "echo 'Container accessible'" >/dev/null 2>&1; then
        success "LXC container '$container_name' exists and is accessible"
        return 0
    else
        warn "LXC container '$container_name' not accessible"
        return 1
    fi
}

# Function to create LXC container via Terraform
create_lxc_container() {
    local target_host="$1"
    
    log "Creating LXC container for '$MODULE' via Terraform..."
    
    if [[ "$DRY_RUN" == "true" ]]; then
        dry_run_log "Would create LXC container for '$MODULE'"
        return 0
    fi
    
    # Check if we have a Proxmox Terraform configuration
    if [[ -d "terraform/proxmox" ]]; then
        cd terraform/proxmox
        
        # Initialize if needed
        if [[ ! -d ".terraform" ]]; then
            log "Initializing Proxmox Terraform..."
            terraform init
        fi
        
        # Plan and apply
        log "Planning LXC container creation..."
        if terraform plan -var="module_name=$MODULE" -out=lxc-plan.tfplan; then
            log "Applying LXC container creation..."
            if terraform apply lxc-plan.tfplan; then
                success "LXC container created successfully"
                cd ../..
                return 0
            else
            ...
            error "Failed to plan LXC container creation"
            cd ../..
            return 1
        fi
    else
        warn "No Proxmox Terraform configuration found. Please create LXC container manually."
        return 1
    fi
}
...
run_terraform() {
    ...
    log "Step 2: Running Terraform deployment for '$MODULE'"
...
update_secrets() {
    ...
    log "Step 3: Running secret update scripts for '$MODULE'"
...
main() {
    log "Starting orchestrated deployment for module: $MODULE"
    
    # Get target host
    local target_host=$(get_target_host)
    log "Target host: $target_host"
    
    # Step 0: Create LXC container (if not skipped)
    if [[ "$SKIP_LXC" == "false" ]]; then
        if ! check_lxc_exists "$target_host"; then
            if ! create_lxc_container "$target_host"; then
                error "Failed to create LXC container. Exiting."
                exit 1
            fi
        fi
    else
        log "Skipping LXC creation as requested"
    fi
    
    # Step 1: Initial NixOS deployment
    if [[ "$TERRAFORM_ONLY" == "false" ]]; then
        if ! deploy_nixos "$target_host" "1"; then
            error "Initial NixOS deployment failed. Exiting."
            exit 1
        fi
    else
        log "Skipping NixOS deployment (terraform-only mode)"
    fi
    ...
    # Step 4: Final NixOS deployment to apply secrets
    if [[ "$TERRAFORM_ONLY" == "false" ]]; then
        log "Step 4: Final NixOS deployment to apply updated secrets"
        if ! deploy_nixos "$target_host" "4"; then
    ...
    success "Orchestrated deployment completed successfully for '$MODULE'!"
    log "Module '$MODULE' is now fully deployed and configured"
}
