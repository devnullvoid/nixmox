...
MAX_JOBS="${MAX_JOBS:-1}"
# Local AGE key file used for sops decryption on remote (override with env)
...
# Ensure AGE key exists on remote for sops (only if missing)
REMOTE_AGE_DIR="/etc/sops/age"
REMOTE_AGE_FILE="$REMOTE_AGE_DIR/keys.txt"
log "Checking for AGE key on remote: $REMOTE_AGE_FILE"
if ! ssh "$TARGET_HOST" "test -f '$REMOTE_AGE_FILE'" >/dev/null 2>&1; then
    if [[ -f "$SOPS_AGE_KEY_FILE" ]]; then
        log "Remote AGE key not found. Installing key from $SOPS_AGE_KEY_FILE"
        ssh "$TARGET_HOST" "mkdir -p '$REMOTE_AGE_DIR' && chmod 700 '$REMOTE_AGE_DIR'" || {
            error "Failed to create $REMOTE_AGE_DIR on remote"
            exit 1
        }
        if scp -q "$SOPS_AGE_KEY_FILE" "$TARGET_HOST:$REMOTE_AGE_FILE"; then
            ssh "$TARGET_HOST" "chmod 600 '$REMOTE_AGE_FILE' && chown root:root '$REMOTE_AGE_FILE'" || {
                error "Failed to set permissions on $REMOTE_AGE_FILE"
                exit 1
            }
            success "Installed AGE key on remote: $REMOTE_AGE_FILE"
        else
            error "Failed to copy AGE key to remote"
            exit 1
        fi
    else
        warn "Local AGE key not found at $SOPS_AGE_KEY_FILE; skipping remote key install. SOPS decryption on remote may fail."
    fi
else
    success "AGE key already present on remote"
fi
