...
let
  ...
  compareManifestWithState = manifest: currentState:
    let
      ...
      deployedServices = currentState.deployed_services or {};
      terraformContainers = currentState.terraform_state.containers or {};
      authentikResources = currentState.terraform_state.authentik_resources or {};
      nixosDeployments = currentState.nixos_deployments or {};
      ...
      servicesToDeploy = builtins.filter (serviceName:
        let
          ...
          deployedService = deployedServices.${serviceName} or null;
        in
        ...
        (manifestService.version or "1.0") != (deployedService.version or "1.0") ||
        (compareDependencies manifestService.depends_on deployedService.depends_on)
      ...
      containersToCreate = builtins.filter (serviceName:
        let
          manifestService = manifestServices.${serviceName};
          deployedContainer = terraformContainers.${serviceName} or null;
        in
        ...
        manifestService.ip != deployedContainer.ip ||
        manifestService.vmid != deployedContainer.vmid
      ) (builtins.attrNames manifestServices);
      ...
      oidcAppsToCreate = builtins.filter (serviceName:
        let
          manifestService = manifestServices.${serviceName};
          deployedApp = authentikResources.${serviceName} or null;
        in
        # OIDC app needs creation if:
        # 1. Service has OIDC config AND
        # 2. Not in authentik resources OR
        # 3. OIDC config changed
        (manifestService.interface.auth or null) != null &&
        (manifestService.interface.auth.type or "") == "oidc" &&
        (deployedApp == null ||
         manifestService.interface.auth.oidc.client_id != deployedApp.client_id)
      ) servicesToDeploy;
      ...
      nixosRedeployments = builtins.filter (serviceName:
        let
          manifestService = manifestServices.${serviceName};
          deployedNixos = nixosDeployments.${serviceName} or null;
        in
        ...
        deployedNixos == null ||
        (manifestService.version or "1.0") != (deployedNixos.version or "1.0")
      ) servicesToDeploy;
  ...
  compareDependencies = deps1: deps2:
    let
      deps1Sorted = builtins.sort builtins.lessThan (deps1 or []);
      deps2Sorted = builtins.sort builtins.lessThan (deps2 or []);
    in
      ...
      updatedState // {
        deployed_services = currentState.deployed_services // {
          ${serviceName} = {
            ...
            version = manifestService.version or "1.0";
            depends_on = manifestService.depends_on or [];
      ...
      updatedState // {
        terraform_state = currentState.terraform_state // {
          authentik_resources = currentState.terraform_state.authentik_resources // {
            ${serviceName} = {
              created_at = timestamp;
              client_id = manifestService.interface.auth.oidc.client_id;
              client_type = manifestService.interface.auth.oidc.client_type;
            };
          };
        };
      }
  ...
  generateExecutionOrder = manifest: servicesToDeploy:
    let
      ...
      dependencyGraph = builtins.listToAttrs (map (serviceName: {
        name = serviceName;
        value = manifest.services.${serviceName}.depends_on or [];
      }) servicesToDeploy);
    ...
    let
      ...
      checkCircularDeps = serviceList:
        let
          hasCircular = serviceName:
            let
              deps = manifest.services.${serviceName}.depends_on or [];
              visited = [serviceName];
            in
          ...
          checkDeps = deps: visited:
            if deps == [] then false
              ...
              if builtins.elem dep visited then
                ...
                (manifest.services.${dep}.depends_on or [] != [] &&
                 checkDeps manifest.services.${dep}.depends_on (visited ++ [dep]));
        in
      ...
      checkMissingDeps = serviceList:
        builtins.any (serviceName:
          let deps = manifest.services.${serviceName}.depends_on or []; in
