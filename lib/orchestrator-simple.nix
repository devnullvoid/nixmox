...
let
  ...
  generateExecutionOrder = corePhases: servicePhases: let
    ...
    serviceOrder = lib.concatLists (lib.mapAttrs (serviceName: servicePhaseDefs) servicePhases);
  ...
  generateDeploymentScripts = manifest: plan: let
    # Core phase scripts
    coreScripts = lib.mapAttrs (phaseName: phaseDef) plan.phases;
    
    # Service phase scripts
    serviceScripts = lib.mapAttrs (serviceName: servicePhaseDefs) plan.service_phases;
    
    # Generate Terraform scripts for core phases
    terraformCoreScripts = lib.mapAttrs (phaseName: phaseDef) (lib.filterAttrs (filterName: filterPhaseDef: filterPhaseDef.terraform) plan.phases);
    
    # Generate NixOS scripts for core phases
    nixosCoreScripts = lib.mapAttrs (phaseName: phaseDef) (lib.filterAttrs (filterName: filterPhaseDef: filterPhaseDef.nixos) plan.phases);
    
    # Generate service scripts
    serviceDeploymentScripts = lib.mapAttrs (serviceName: servicePhaseDefs) plan.service_phases;
  in
    {
      inherit coreScripts;
  ...
  generateMainOrchestratorScript = manifest: plan: scripts: let
    mainScript = pkgs.writeShellScript "nixmox-orchestrator" ''
      ...
      ${lib.concatStringsSep "\n" (lib.mapAttrs (serviceName: servicePhaseDefs: ''
        echo "Deploying service: ${serviceName}"
        CURRENT_PHASE="${serviceName}"
        
        # Terraform Service Infrastructure
        if [ -f "${servicePhaseDefs.terraform_infra}"]; then
          echo "  - Terraform infrastructure..."
          ${servicePhaseDefs.terraform_infra}
        fi
        
        # NixOS Service Configuration
        if [ -f "${servicePhaseDefs.nixos_config}"]; then
          echo "  - NixOS configuration..."
          ${servicePhaseDefs.nixos_config}
        fi
        
        # Terraform Service Configuration
        if [ -f "${servicePhaseDefs.terraform_config}"]; then
          echo "  - Terraform configuration..."
          ${servicePhaseDefs.terraform_config}
        fi
        
        # Service Verification
        if [ -f "${servicePhaseDefs.verify}"]; then
          echo "  - Verifying service..."
          ${servicePhaseDefs.verify}
        fi
